...
void main() {
  ...
  vec4 pixel = texture2D(uImage, aUV);
  vColor = pixel.rgb;

  float luminance =
    dot(pixel.rgb, vec3(0.2126, 0.7152, 0.0722));

  float imageAlpha = pixel.a;

  /*
   * Convert image UV into portrait coordinates.
   */
  vec2 p = vec2(
    (aUV.x - 0.5) * uImageAspect,
    (aUV.y - 0.5)
  );
  ...
  float movementY =
    cos(uTime * 0.65 + aRandom * 23.0);

  p.x += movementX * uSpread * 0.0005;
  p.y += movementY * uSpread * 0.0005;

  /*
   * Tiny Z movement creates depth.
   */
  float depth =
    sin(aRandom * 30.0 + uTime) *
    0.015;
  ...
  float force = smoothstep(0.45, 0.0, distanceToMouse);
   ...
   * Do not fade out dark areas (like eyes/sunglasses).
...
void main() {
   ...
   * for accurate detail in dark mode.
   */
  vec3 particleColor = vColor * 1.3;
...
export default function VishnuParticlesDark({ src }: { src: string }) {
  ...
  useEffect(() => {
     ...
     * RENDERER
     *
     * Alpha = true is important.
     *
     * The canvas itself has NO background.
     */
    const renderer =
    ...
    const texture =
      loader.load(
        ...
        (loadedTexture) => {
           ...
           * Shader calculates their
           * actual portrait positions.
           */
